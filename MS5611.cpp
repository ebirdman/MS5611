...
Changes by Vladimir Blanshey Nov. 2016:
- Arduino ifdefs for Versions below v1.6 removed
...
{
//    Wire.begin(); moved to the user code only
...
double MS5611::getAltitudeMeters(double pressure, double seaLevelPressure)
{
    return (44330.0f * (1.0f - pow((double)pressure / (double)seaLevelPressure, 0.1902949f)));
}

// Calculate altitude in Feet from Pressure in Pa & Sea level pressure in Pa
double MS5611::getAltitudeFeet( double pressure, double slp )
{
	double altOutput = 0;	
	altOutput = getAltitudeMeters(pressure, slp) * 3.28084;
	return altOutput;
}
